...
from buttons.button import StartBTN, contactBTN, priceBTN, statusBTN, adminBTN
...
async def help_handler(message: Message, state: FSMContext):
    await message.answer("""Bu bot vazifalarni qabul qilish uchun ishlaydi!\n"
Agar sizda qandaydur savol tug'ilsa adminlarimizga murojaat qiling!\n\n""",
    reply_markup=adminBTN)

    await state.set_state(Question.answer)

@dp.message(Question.answer)
async def question_prompt_handler(message: types.Message, state: FSMContext):
...
async def question_handler(message: Message, state: FSMContext):
    data = message.text
    await message.bot.send_message(chat_id=1509198141, text=f"""{message.from_user.full_name} dan savol bor‼:\n\n{data}""")
    await message.reply(f"""🎫Savolingiz adminlarga yuboridi. Tez oraada javob olasiz! ;)""")
    await state.clear()
